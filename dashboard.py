...
(load_config, get_pool_status, check_dependencies). Pestañas:
  ...
  📤 Subir       — cola de vídeos largos pendientes; sube a YouTube EN PRIVADO
                   con el OK de Diego (los shorts no: 51 subidas no caben en cuota)
...
# conexiones + hub que pulsa). Vive en assets/roadmap.html y se embebe con
# st.iframe: necesita el iframe para que las animaciones CSS/SMIL funcionen,
# y `st.components.v1.html` quedó deprecado (Streamlit 1.57 remite a st.iframe).
_ROADMAP_HTML = os.path.join("assets", "roadmap.html")
...
(
    ...
    tab_resultados, tab_subir, tab_competencia, tab_config,
) = st.tabs(
    ["🗺️ Roadmap", "📊 Estado", "🎬 Operar", "📡 Progreso",
     "🖼️ Resultados", "📤 Subir", "🔍 Competencia", "⚙️ Config"]
)
...
with tab_roadmap:
    ...
    st.iframe(_ROADMAP_HTML, height=470)
...
with tab_subir:
    ...
    from modules.competitor_scout import QuotaExhausted as _QuotaExhausted

    _cuota_ok, _cuota_msg = _yt.puede_subir(config)
    (st.info if _cuota_ok else st.error)(f"Cuota de YouTube — {_cuota_msg}")

    if not _yt.hay_token(config):
        st.warning(
            "Sin autorizar todavía. La primera subida abrirá el navegador para "
            "que des permiso con tu cuenta de YouTube; después se recuerda."
        )
        with st.expander("Cómo crear las credenciales (una sola vez)"):
            st.code(_yt.credentials_help(config), language="text")

    _cola = _yt.pendientes(config)
    if not _cola:
        st.info("No hay vídeos en `output/`. Genera uno desde la pestaña Operar.")
    ...
    for _item in _cola:
        ...
        with _c2:
            ...
            if st.button("📤 Subir en privado", key=f"subir_{_item['stem']}", disabled=_dis):
                ...
                def _prog(hechos, total, _b=_barra):
                    _b.progress(hechos / total,
                                text=f"Subiendo… {hechos/1024/1024:.0f} / {total/1024/1024:.0f} MB")
                ...
                except _yt.FaltaCredencial as e:
                    st.error(str(e))
                except Exception as e:
                    st.error(f"La subida falló: {e}")
                    st.caption(
                        "El vídeo sigue en `output/` y no se ha marcado como subido: "
                        "puedes reintentarlo."
                    )
